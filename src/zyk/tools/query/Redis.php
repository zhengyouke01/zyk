...
// |【系统为商业系统非开源，注意保密】
// +----------------------------------------------------------------------
// | Copyright (c) 2018 bcard All rights reserved.
// +----------------------------------------------------------------------
// | desc: redis操作类
// +----------------------------------------------------------------------

namespace zyk\tools\query;
...
class Redis implements BaseInterface {
    ...
    public static function getInstance($config = [], $attr = array()){
        ...
        } elseif( time() > static::$_instance[$k]->expireTime || static::$_instance[$k]->status != 1) {
    ...
    /**
     * 为 hash表设定一个字段的值,如果字段存在，返回false
     * @param string $key 缓存key
     * @param string  $field 字段
     * @param string $value 值。
     * @return bool
